...
The __Age Range__ variable was difficult to work with because age ranges varied so differently across advertisements. Moving forward, it would be best to repeat this analysis by using dummy variables, separating the ages into different ranges (for example: 0-18, 19-29, 30-45, 45+) and marking each ad with a 1 if it falls within a certain range/category. While this would likely provide a more accurate idea of how different the various age categories might be (more so than the redefined scale I created here), it is still complicated because different advertisers have different age range specifications. For example, if the categories are 0-18, 19-29, 30-45, and 45+, one advertiser may be targeting users ages 17-19 while another may target users ages 0-33; the ranges would not fall cleanly into the different categories and may require weighting by age to be more accurate. The next step would be to compare the different age categories to determine if any of them, individually, are significant.
...
Ultimately, more research needs to be done to consider the other extenuating factors for 2020, too - things like targeted geographic location, how long users looked at the ads, and the age ranges (consider a 17-19 age range and a 17-40 age range, which both received the same score of 1.5). Would the fact that 2020 is a presidential election year have any bearing on the results? Additional data I'd like to look at, especially to answer this aforementioned presidential election question, would be to look at ad spend per political party and by state/geographic region in the United States to see if that has any effect on the results. Comparing it to a more "control" year like 2019 (which still had some elections, but not midterm elections or a presidential election) may provide insight into what is "average" versus what is "extraordinary" in terms of ad spend.

If I were running a campaign/program that advertised on Snap, my results would inform my budget for the advertisement (__Spend__), how long the advertisement would run for on Snapchat (__Run Time__), and who might be the best target audience on this platform (__Age Range__) in order to maximize impressions while staying cost-efficient. It would help inform how much I should invest in advertising on Snapchat and how much I might want to allocate to other marketing opportunities, such as other paid social media ads (like on Facebook or Instagram) or television ads (if applicable) or anything else. This would help support/supplement my marketing plan and give me more freedom, in a sense, with which to work to advertise my product. 
